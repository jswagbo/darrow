...
- **Tailwind CSS v4** - Utility-first CSS framework with modern CSS-first configuration
- **TypeScript** - Full type safety throughout the application

### File Structure & Routing
- File-based routing in `src/routes/` directory
- Root layout defined in `src/routes/__root.tsx` with theme provider and global components
- Marketing routes grouped under `src/routes/(marketing)/`
- Route files must use `.tsx` extension for proper TypeScript integration

### Component Architecture
- UI components in `src/components/ui/` following shadcn/ui patterns
- Shared components in `src/components/`
- Uses path alias `~/` pointing to `src/` directory
- Theme system with light/dark mode support via context provider

### State Management
- TanStack Query integrated at router level for server state
- Router context provides QueryClient instance to all routes
- Theme state managed through context provider

### Styling System
- Tailwind CSS v4 with CSS-first configuration 
- Global styles in `src/styles/app.css` and `src/styles/custom.css`
- Component variants using class-variance-authority
- Icon system using lucide-react

### Type Safety
- Strict TypeScript configuration with path mapping
- Router type safety through module augmentation
- Component props fully typed with React 19
...
Components are configured to use the `~/components` alias and follow the New York style variant.
